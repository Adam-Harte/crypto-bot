  ...
  if (adxInput.close.length > adxInput.period) {
    ...
    if (latestAdx && latestAdx.adx > 25 && latestAdx.pdi > latestAdx.mdi) {
      ...
      if (!inPositionAdx) {
        ...
        try {
          fs.writeFileSync('C:/Users/adamh/Desktop/crypto-bot/logs/adx.txt', `buy at ${close}` + "\n", { flag: 'a+' });
        } catch (err) {
            console.error(err)
        }
    ...
    if (latestAdx && latestAdx.adx > 25 && latestAdx.pdi < latestAdx.mdi) {
      ...
      if (inPositionAdx) {
        ...
        try {
          fs.writeFileSync('C:/Users/adamh/Desktop/crypto-bot/logs/adx.txt', `sell at ${close}` + "\n", { flag: 'a+' });
        } catch (err) {
            console.error(err)
        }
        ...
        inPositionAdx = false;
        try {
          fs.writeFileSync('C:/Users/adamh/Desktop/crypto-bot/logs/adx.txt', `profit at ${profit}` + "\n", { flag: 'a+' });
        } catch (err) {
            console.error(err)
        }
      }
    ...
    if (latestAdx && latestAdx.adx < 25 && latestAdx.pdi < 25 && latestAdx.mdi < 25) {
      ...
      try {
        fs.writeFileSync('C:/Users/adamh/Desktop/crypto-bot/logs/adx.txt', "non trending market" + "\n", { flag: 'a+' });
      } catch (err) {
          console.error(err)
      }
    }
